#!/usr/bin/env pwsh
# PowerShell backup script - simplified for Windows
...
function Send-Notification {
    ...
    try {
        ...
        $body = @{ chat_id = $env:TELEGRAM_CHAT_ID; text = "$Title`n$Message" }
        Invoke-RestMethod -Uri "http://127.0.0.1:18789/channel/telegram/send" -Method Post -Headers $headers -Body $body -TimeoutSec 10 | Out-Null
    } catch {
        Log "WARN: Telegram send failed: $_"
    }
}
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($env:OPENCLAW_TOKEN)) { throw "OPENCLAW_TOKEN not set" }
    if ([string]::IsNullOrWhiteSpace($env:TELEGRAM_CHAT_ID)) { throw "TELEGRAM_CHAT_ID not set" }
    ...
    if (-not $status) {
        ...
        Send-Notification "Backup Report" "No new conversation today. Backup checked."
        exit 0
    }
    ...
    # Notification
    $msg = "Backup successful`nTime: $TIMESTAMP`nDuration: ${duration_sec}s"
    Send-Notification "Backup OK" $msg
